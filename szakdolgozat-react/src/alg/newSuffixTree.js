...
class SuffixTree {
  ...
  searchPattern(pattern) {
    ...
    while (currentNode) {
      const edge = currentNode.next[pattern[currentEdgePos]];

      if (!edge) {
        // Character not found in the current edge
        return false;
      }

      const edgeStr = this.edgeString(edge);
      const patternRemaining = pattern.slice(currentEdgePos);

      // Find the common prefix between the edge string and the remaining pattern
      let commonPrefix = 0;
      while (
        commonPrefix < edgeStr.length &&
        commonPrefix < patternRemaining.length &&
        edgeStr[commonPrefix] === patternRemaining[commonPrefix]
      ) {
        commonPrefix++;
      }

      if (commonPrefix === patternRemaining.length) {
        // Entire pattern is found
        return true;
      }

      if (commonPrefix === edgeStr.length) {
        // Entire edge is consumed, move to the next node
        currentNode = edge;
        currentEdgePos += commonPrefix;
      } else {
        // Pattern partially matches the edge
        return false;
      }
    }
